  ...
  ({ variant = 'primary', children, ...rest }, ref) => {
    const cn = clsx(
      'isolate text-sm group rounded-full shrink-0 px-4 py-2 transition-colors transition-shadows overflow-hidden relative outline-offset-2 outline-accent',
      {
        ...
        'bg-slate-100 text-foreground': variant === 'secondary',
    ...
    return (
      <RACButton {...rest} className={cn} ref={ref}>
        <span className="transition-all absolute inset-0 opacity-0 -z-10 group-data-[hovered]:opacity-20 group-data-[pressed]:opacity-40 bg-stone-500 rounded-none" />
        {children}
      </RACButton>
    );
  }
